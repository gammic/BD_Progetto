...
def hi_utility_rec_couch(base_url, n):
    ...
    for x in range(1, n + 1):
        start = time.time()
        # Filtro per recensioni con un denominatore maggiore di 0 (evita divisioni per zero)
        ...
        # Aggiunta del calcolo del rapporto di utilità (HelpfulnessRatio)
        for item in results:
            item['value'][0] = item['value'][0] / item['value'][1] if item['value'][1] != 0 else 0
            if item['value'][0] > 1.00:
                results.remove(item)
        # Ordinamento per rapporto di utilità e selezione delle prime 10 recensioni
...
def hi_val_prods_couch(base_url, n):
    ...
    for x in range(1, n + 1):
        start = time.time()
        # View per ottenere la media dei punteggi e il conteggio delle recensioni per ogni prodotto
        ...
        # Filtro per prodotti con almeno 50 recensioni
        filtered_results = [item for item in results if item['value']['count'] >= 50]

        # Ordinamento in base alla media del punteggio
...
def temp_trend_couch(base_url, n):
    ...
    for x in range(1, n + 1):
        start = time.time()
        # View per ottenere il numero di recensioni per data
...
def len_rec_val_couch(base_url, n):
    ...
    for x in range(1, n + 1):
        start = time.time()
        # View per ottenere la lunghezza media delle recensioni per valutazione
